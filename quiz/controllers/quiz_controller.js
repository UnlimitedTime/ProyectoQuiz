...
//Opciones de subidas a Cloudinary
...
exports.index= function(req, res, next){
	...
	if(textoBuscar !== ""){
		for(var i=0; i<textoBuscar.length; i++){
			if(textoBuscar[i] === " "){
				textoBuscar[i]="%";
			}
		}
		textoBuscar = "%" + textoBuscar + "%";
		models.Quiz.findAll({ include: [models.Attachment], where: {question: {$like: textoBuscar}}}).then(
			function(quizzes){
...
exports.destroy = function(req, res, next){
	...
	//Borra la imagen de Cloudinary (Ignoro el resultado)
...
exports.format = function(req, res, next){
	models.Quiz.findAll().then(function(quizzes){
		res.render("quizzes/format.ejs", {quizzes: quizzes});
	});
}

//GET /quizzes/:id.format
exports.format2 = function(req, res, next){
	models.Quiz.findById(req.params.quizId).then(function(quiz){
		if(quiz){
			var answer = req.query.answer || "";
			res.render("quizzes/format2.ejs", {quiz: req.quiz, answer: answer});
		}else{
			throw new Error("No existe la pregunta solicitada");
		}
	}).catch(function(error){
		next(error);
	});	
...
exports.ownershipRequired = function(req, res, next){
	...
	if( isAdmin || quizAuthorId === loggedUserId){
		...
		console.log("Operación prohibida: El usuario logueado no es el administrador ni el autor del quiz");
		res.send(403);	
	}
};

/**Función solicitada para el Tema 27 **/
